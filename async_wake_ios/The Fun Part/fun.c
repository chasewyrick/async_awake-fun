...
//		printf("%s\n",name);
    ...
    // MARK: - Bootstrap
    // Inspired heavily by yalu 102. Code cleaned up a bit.
    {
        //int signature = open("/.installed_someFun", O_RDONLY);
        int signature = -1;
        
        if (signature == -1) {
            printf("We haven't installed our bootstrap yet.\n");
            
            // Unlink old files
            unlink("/bin/tar");
            unlink("/bin/launchctl");
            
            // Copy our tar binary and set permissions on it
            cp("/bin/tar", tarPath());
            chmod("/bin/tar", 0777);
            ...
            // Copy launchctl into place
            cp("/bin/launchctl", launchctlPath());
            chmod("/bin/launchctl", 0755);
            
            // Make flag file
            open("/.installed_someFun", O_RDWR|O_CREAT);
            // Make sure cydia doesn't stash
            open("/.cydia_no_stash", O_RDWR|O_CREAT);
            
            // Copy reload into place
            unlink("/usr/libexec/reload");
            cp("/usr/libexec/reload", reloadPath());
            chmod("/usr/libexec/reload", 0755);
            chown("/usr/libexec/reload", 0, 0);
            
            // Copy our launch daemon into place
            unlink("/Library/LaunchDaemons/0.reload.plist");
            cp("/Library/LaunchDaemons/0.reload.plist", reloadPlist());
            chmod("/Library/LaunchDaemons/0.reload.plist", 0644);
            chown("/Library/LaunchDaemons/0.reload.plist", 0, 0);
            
            // Copy dropbear launch daemon into place
            unlink("/Library/LaunchDaemons/dropbear.plist");
            cp("/Library/LaunchDaemons/dropbear.plist", dropbearPlist());
            chmod("/Library/LaunchDaemons/dropbear.plist", 0644);
            chown("/Library/LaunchDaemons/dropbear.plist", 0, 0);
            
            // Run uicache
            // FIXME: - This is broken in iOS 11.
            posix_spawn(&pd, "/usr/bin/uicache", 0, 0, NULL, NULL);
        }
